...
int MythSocket::InitalSocket()
{
	curl = curl_easy_init();
	return 0;
}
int MythSocket::BindAddressPort(const char* ip, u_short port)
{
	//wait_on_socket()
	if (curl){ 
...
int MythSocket::SendStr(const char* str, int length)
{
	if (curl){
		size_t iolen;
		if (length == -2){
...
int MythSocket::ReceiveData(char* recvBuf, int recvLength)
{
	size_t iolen;
	if (curl){
...
int MythSocket::cmp(char* buff, char*str, int length){
#if 1
...
int MythSocket::ReceiveDataLn(char* recvBuf, int recvLength, char* lnstr){
	...
	while (1){
		SDL_PollEvent(NULL);
		SDL_Delay(1);
		len = ReceiveData(recv, rlength);
		if (len > 0){
			for (i = 0; i < len - tmplength; i++){
				if (cmp(&recv[i], lnstr, tmplength) == 0){
					...
					if (contentlength > 0){
						...
						while (contentlength > 0){
							len = ReceiveData(recvBuf + tmpptr, contentlength);
							tmpptr += len;
							contentlength -= len;
						}
						return returnvalue;
...
MythSocket* MythSocket::CreateNew(const char* ip, u_short port){
